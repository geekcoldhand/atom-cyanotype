...
export function renderToBlob(img, s) {
  return new Promise((resolve, reject) => {
    ...
    // ── 5. lightWash — top-down architectural gradient (screen) ───
    {
      ...
      g.addColorStop(0,    `rgba(230,247,255,${Math.min(1, 0.2  + lightWash * 0.008)})`)
      g.addColorStop(0.35, `rgba(207,239,255,${Math.min(1, 0.1  + lightWash * 0.006)})`)
      g.addColorStop(0.65, `rgba(111,186,217,${Math.min(1, 0.02 + lightWash * 0.002)})`)
    ...
    // ── 6. highlightLift — top-of-frame radial screen ─────────────
    {
      ...
      g.addColorStop(0,   `rgba(255,255,255,${Math.min(1, highlightLift * 0.012)})`)
      g.addColorStop(0.5, `rgba(207,239,255,${Math.min(1, highlightLift * 0.007)})`)
      g.addColorStop(1,   'rgba(0,0,0,0)')
      ctx.globalCompositeOperation = 'screen'
    ...
    // ── 7. reflection — diagonal glass shimmer (overlay) ──────────
    {
      ...
      g.addColorStop(0,   `rgba(255,255,255,${Math.min(0.9, 0.05 + reflection * 0.006)})`)
      g.addColorStop(0.5, `rgba(207,239,255,${Math.min(0.9, 0.03 + reflection * 0.004)})`)
      g.addColorStop(1,   `rgba(255,255,255,${Math.min(0.9, 0.04 + reflection * 0.005)})`)
    ...
    // ── 9. shadowControl — edge vignette (multiply) ────────────────
    {
      ...
      g.addColorStop(1, `rgba(10,26,47,${Math.min(0.8, 0.05 + (100 - shadowLift) * 0.003)})`)
      ctx.globalCompositeOperation = 'multiply'
      ctx.globalAlpha = 0.7
      ctx.fillStyle   = g
      ctx.fillRect(0, 0, W, H)
